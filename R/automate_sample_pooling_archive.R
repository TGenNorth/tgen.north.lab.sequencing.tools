...
#'
#' @description ARCHIVE This function is meant to automate sample pooling for sequencing. The process will produce several files including pooling maps, theoretical concentrations, and a script that can be added to the Epmo for completely automated pooling.
...
#' @export automate_sample_pooling_archive
#' @import tidyr
#' @import tidyverse
#' @import ggplot2
#' @import dplyr

automate_sample_pooling_archive <- function(Quant, Date_Project, Max_Volume = 8, Min_Volume = 2){
  ...
  if (length(unique(PlateMap$Plate)) < 7 & length(unique(PlateMap$Pool)) < 25 & max(Theoretical_Pools$Final_Volume) < 1500 & Max_Volume <= 10){
  ...
  } else { print("Error in Epmo program... Check to make sure you have less than 7 plates, less than 25 pools, max pool size <1500ul, and max_pool volume <10ul per sample!")}
